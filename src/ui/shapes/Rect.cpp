...
namespace vanadium::ui::shapes {
	...
	RectShapeRegistry::RectShapeRegistry(const graphics::RenderContext& context, VkRenderPass uiRenderPass,
		...
		uint32_t pipelineId = context.pipelineLibrary->findGraphicsPipeline("UI Rect");
		context.pipelineLibrary->createForPass(uiRenderPassSignature, uiRenderPass, { pipelineId });
		m_setAllocationInfo = { .typeInfos = { { .type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, .count = 1 } },
	...
	void RectShapeRegistry::allocateBuffer() {
		...
		m_shapeDataTransfer = m_context.transferManager->createTransfer(
			m_maxShapeDataCapacity * sizeof(ShapeData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
	...
	void RectShapeRegistry::addShape(Shape* shape, uint32_t childDepth) {
		...
		auto iterator = std::lower_bound(m_shapes.begin(), m_shapes.end(),
	...
	void RectShapeRegistry::renderShapes(VkCommandBuffer commandBuffer, uint32_t frameIndex, uint32_t childDepth,
		...
		auto rangeIterator = std::lower_bound(
			m_shapes.begin(), m_shapes.end(), RectShapeRegistry::RegistryEntry{ .childDepth = childDepth },
			childDepthComparator);
		auto rangeEndIterator = std::lower_bound(
			m_shapes.begin(), m_shapes.end(), RectShapeRegistry::RegistryEntry{ .childDepth = childDepth + 1 },
			childDepthComparator);
