...
namespace vanadium::ui::shapes {
	...
	RectShapeRegistry::RectShapeRegistry(UISubsystem*, const graphics::RenderContext& context, VkRenderPass uiRenderPass,
	...
	void RectShapeRegistry::renderShapes(VkCommandBuffer commandBuffer, uint32_t frameIndex, uint32_t childDepth,
		...
		auto rangeEndIterator = std::lower_bound(m_shapes.begin(), m_shapes.end(),
												 RectShapeRegistry::RegistryEntry{ .childDepth = childDepth + 1 },
		...
		vkCmdDraw(commandBuffer, 5, elementCount, 0, elementOffset);
	}
