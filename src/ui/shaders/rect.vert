...
void main() {
    ...
    gl_Position = vec4((rotation * (positions[indices[gl_VertexIndex % 8]] * data[instanceIndex].size)) / targetDimensions + data[instanceIndex].position.xy / targetDimensions, data[instanceIndex].position.z, 1.0f);
    gl_Position *= 2.0f;
    gl_Position -= 1.0f;
